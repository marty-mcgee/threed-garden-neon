...
Database schemas are co-located with their features. Auth-related schemas (auto-generated by better-auth) are in `src/lib/schema.ts`. You can add additional schema files for other features and they will be automatically detected by Drizzle Kit.
...
1. Update your schema files (e.g., `src/lib/schema.ts` or other feature-specific schema files)
